...
def send_msg_check(ch_id, u_id):
    if data_store.has_channel_id(ch_id) and data_store.get_channel(ch_id).has_member_id(u_id):
        return True
    if data_store.has_dm_id(ch_id) and data_store.get_dm(ch_id).has_member_id(u_id):
        return True
    return False
