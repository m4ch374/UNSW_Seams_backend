...
def test_channels_create_error_pub_and_priv(get_token_1):
    for s in ERROR_LIST:
        ...
        resp1 = requests.post(ENDPOINT_CREATE_CHNL, json=data1)
        resp2 = requests.post(ENDPOINT_CREATE_CHNL, json=data2)
...
def test_channels_create_public(get_token_1):
    for i, name in enumerate(NAMES_LIST):
        ...
        val = requests.post(ENDPOINT_CREATE_CHNL, json=data).json()
...
def test_channels_create_pub_and_priv(get_token_1):
    for i, name in enumerate(NAMES_LIST):
        ...
        val = requests.post(ENDPOINT_CREATE_CHNL, json=data).json()
        val_1 = requests.post(ENDPOINT_CREATE_CHNL, json=data_1).json()
