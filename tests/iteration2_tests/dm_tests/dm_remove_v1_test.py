...
# Local helper
def generate_dm_remove_json(tok, dm_id):
    return {
        'token': tok,
        'dm_id': dm_id
    }
...
def test_invalid_token(get_token_1):
    ...
    data = generate_dm_remove_json(INVALID_TOKEN, dm_id)
    resp = requests.delete(ENDPOINT_DM_REMOVE, json=data)

    assert resp.status_code == AccessError.code

def test_invalid_dm_id(get_token_1):
    data = generate_dm_remove_json(get_token_1, INVALID_DM_ID)
    resp = requests.delete(ENDPOINT_DM_REMOVE, json=data)
...
def test_not_member(get_token_1, get_token_2):
    ...
    data = generate_dm_remove_json(get_token_2, dm_id)
    resp = requests.delete(ENDPOINT_DM_REMOVE, json=data)
    assert resp.status_code == AccessError.code
...
def test_not_owner(get_token_1, get_usr_2):
    ...
    data = generate_dm_remove_json(get_usr_2['token'], dm_id)
    resp = requests.delete(ENDPOINT_DM_REMOVE, json=data)
    assert resp.status_code == AccessError.code
...
def test_valid(get_usr_1, get_usr_2):
    ...
    data = generate_dm_remove_json(get_usr_1['token'], dm_id)
    ...
    data = generate_dm_remove_json(get_usr_2['token'], dm_id1)
    requests.delete(ENDPOINT_DM_REMOVE, json=data)
