"""
This test for the behaviours of
/dm/details/v1
"""
...
# Import helpers
from tests.iteration2_tests.helper import generate_dm_json, generate_dm_input_json
...
def test_global_owner_has_no_permission(get_token_1, get_token_2):
    data = generate_dm_input_json(get_token_2, [])
    dm_id = requests.post(ENDPOINT_DM_CREATE, json=data).json()['dm_id']

    data = generate_dm_json(get_token_1, dm_id)
    resp = requests.get(ENDPOINT_DM_DETAILS, data)
    assert resp.status_code == AccessError.code
