...
from tests.iteration3_tests.endpoints import (
    ENDPOINT_MESSAGE_UNPIN, ENDPOINT_MESSAGE_PIN, ENDPOINT_JOIN_CHNL,
    ENDPOINT_MESSAGE_SEND, ENDPOINT_CHANNEL_MESSAGE,
)
...
def test_message_pin_v1_already_pinned(user_1_made_channel, get_usr_2):
    ...
    # try to unpin a message that is already not pinned
    pin_json_input = create_msg_pin_input_json(user1_token, msg_id)
    response = requests.post(ENDPOINT_MESSAGE_UNPIN, json = pin_json_input)
...
def test_message_unpin_v1_not_an_owner(user_1_made_channel, get_usr_2):
    # create a chnl, then join a second user and send a message
    chnl_id1 = user_1_made_channel['channel']
    user2_token = get_usr_2['token']
    chnl_join_json = create_chnl_join_input_json(user2_token, chnl_id1)
...
# unpin a message with no problems (since original owner of chnl)
def test_message_unpin_v1_success_1(user_1_made_channel, get_usr_2):
    # create a chnl, then join a second user and send a message
    user1_token = user_1_made_channel['token']
    chnl_id1 = user_1_made_channel['channel']
    user2_token = get_usr_2['token']
...
def test_message_unpin_v1_success_2(get_usr_2, user_1_made_channel):
    # create a chnl, then join a second user and send a message
    user1_token = user_1_made_channel['token']
    chnl_id1 = user_1_made_channel['channel']
    user2_token = get_usr_2['token']
    ...
    # unpin the message that just got pinned
    pin_json_input = create_msg_pin_input_json(user2_token, msg_id)
    response = requests.post(ENDPOINT_MESSAGE_UNPIN, json = pin_json_input)
    assert response.status_code == 200
