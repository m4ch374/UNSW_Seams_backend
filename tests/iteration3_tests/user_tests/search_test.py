import requests
from tests.iteration3_tests.user_tests.definitions import REGISTER_V2, SEARCH, ENDPOINT_CHNL_INVITE, ENDPOINT_CREATE_CHNL, ENDPOINT_MESSAGE_SEND, ENDPOINT_DM_CREATE, ENDPOINT_DM_SEND
...
def test_valid_input_ch():
    ...
    user2 = requests.post(REGISTER_V2, json = {'email': 'z2@ed.unsw.edu.au', 'password': '1234567', 'name_first': '22', 'name_last': '22'}).json()
    user_data = user.json()
    ...
    requests.get(SEARCH, {'token': user2['token'], 'query_str': 'q'})
