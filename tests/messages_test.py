...
def test_invalid_channel_id_1(initialise_user_and_channel):
    with pytest.raises(InputError):
        assert channel_messages_v1(1, 5, 0)

# Test for invalid channel id where id is negative
def test_invalid_channel_id_2(initialise_user_and_channel):
    with pytest.raises(InputError):
        assert channel_messages_v1(1, -1, 0)
...
# Test for invalid user id where id is negative
def test_invalid_user(initialise_user_and_channel):
    with pytest.raises(InputError):
        assert channel_messages_v1(-1, 1, 0)
...
# Test that a -1 is returned once the end of messages list is returned
